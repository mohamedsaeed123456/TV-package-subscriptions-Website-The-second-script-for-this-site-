...
$curr=$_POST['curr'];
$result=mysqli_query($link,"update currency set curr='$curr' ");

if($result){
     echo "<script>alert('تم تحديد العملة بنجاح'); 
        	      </script>";
}
    }
...
$result=mysqli_query($link,"Select * from currency");
        ...
        <form method="POST">
            <div class="inputBox">
             <input type="currency" id="msg" value="<?php echo $row['curr'] ?>" name="curr" required="">
             <label>العملة</label>
            </div>
            <input type="submit" name="submit" value="تحديد">
        </form>
    </div>
    <?php } ?>

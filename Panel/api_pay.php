...
$api=$_POST['api'];
$result=mysqli_query($link,"update api_pay set api='$api' ");

if($result){
     echo "<script>alert('تم تعديل api بنجاح'); 
        	      </script>";
}
    }
...
<section class="page-content">
  ...
  <section class="grid">
      ...
      <?php 
include 'connection.php';
$result=mysqli_query($link,"Select * from api_pay");
            while($row =mysqli_fetch_array($result)){
            ?>
         <div class="box">
        
        <h2>تعديل api الدفع</h2>
        
        <form method="POST">
            <div class="inputBox">
             <input type="api" id="api" value="<?php echo $row['api'] ?>" name="api" required="">
             <label>Api</label>
            </div>
